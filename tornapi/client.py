...
class TornAPI:
    ...
    def _call(self, endpoint, params=None):
        """Gets and returns something from Torn's API."""
        now = time.time()
        if now - self.last_request_time < 0.6:
            time.sleep(0.6 - (now - self.last_request_time))
    ...
    def get_user(self, user_id, selections=None):
    ...
    def get_company(self, company_id, selections=None):
    ...
    def get_market(self, market_id, selections=None):
    ...
    def get_torn(self, torn_id, selections=None):
